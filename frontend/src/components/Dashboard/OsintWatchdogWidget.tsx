...
const ACCENT = WIDGET_ACCENT.osint;
...
export function OsintWatchdogWidget() {
  ...
  const refresh = useCallback(async () => {
    try {
      setLoading((prev) => prev && alerts.length === 0 && !stats);
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-3 gap-2 mb-3">
            <MiniStat icon={BarChart3} label="Scans" value={String(stats?.total_scans_today ?? 0)} color={ACCENT} />
            <MiniStat
              icon={Bell}
              label="Alerts"
              value={String(stats?.total_alerts_today ?? 0)}
              color={(stats?.total_alerts_today ?? 0) > 0 ? 'var(--color-warning)' : ACCENT}
            />
            <MiniStat
              icon={Shield}
              label="Watchdog"
